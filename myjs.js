function toggle_like(post_id, type) {
    ...
    let full_icons = {"heart": "fa-heart", "star": "fa-star", "like": "fa-thumbs-up"};
    let empty_icons = {"heart": "fa-heart-o", "star": "fa-star-o", "like": "fa-thumbs-o-up"};
...
function time2str(date) {
    ...
    if (time < 60) {
        return parseInt(time) + "mins ago"
    }
    time = time / 60  // 시간
    if (time < 24) {
        return parseInt(time) + "hrs ago"
    }
    time = time / 24
    if (time < 7) {
        return parseInt(time) + "days ago"
    }
    return `${date.getFullYear()}yr ${date.getMonth() + 1}mn ${date.getDate()}dy`
}
...
function num2str(count) {
    ...
    if (count > 500) {
        return parseInt(count / 100) / 10 + "k" //소숫점 첫째자리까지 가는 표현
    }
    if (count == 0) {
        return ""
    }
